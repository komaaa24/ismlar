...
export class BotService {
  ...
  private ensurePersonalizationSession(ctx: BotContext, reset = false): FlowState {
    if (reset || !ctx.session.flow || (ctx.session.flow as FlowState).name !== 'personalization') {
      ctx.session.flow = {
  ...
  private async startPersonalizationFlow(ctx: BotContext): Promise<void> {
    this.ensurePersonalizationSession(ctx, true);
    ctx.session.quizAnswers = undefined;
    ctx.session.quizTags = undefined;
    const keyboard = new InlineKeyboard()
